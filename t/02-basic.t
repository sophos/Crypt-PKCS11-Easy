...
test slots => sub {
    ...
    my $slot2;
    is(
        exception { $slot2 = $self->pkcs11->get_slot(id => 0x750b0816) },
...
test get_mechs => sub {
    ...
    my $mechs = $self->pkcs11->get_mechanisms(0x750b0816);
    isa_ok $mechs, 'HASH';
...
test signing_and_verifying => sub {
    ...
    my $pkcs11 =
      $self->_new_pkcs11(key => 'signing_key', token => 'test_keys_1');
